...
const COMMAND = `functions debug ${'<functionName>'.yellow} ${'[options]'.yellow}`;
const DESCRIPTION = `Enables debugging for the specified function using Node's ${'--debug'.bold} flag.`;
const USAGE = `Usage:
  ${COMMAND.bold}
...
Positional arguments:
  ${'functionName'.bold}
    The name of the function for which to enable debugging via ${'--debug'.bold}.`;

/**
 * http://yargs.js.org/docs/#methods-commandmodule-providing-a-command-module
 */
exports.command = 'debug <functionName>';
...
exports.builder = (yargs) => {
  yargs
    ...
    .options(_.merge({
      ...
      port: {
        default: 5858,
        description: `The port for the Node.js ${'--debug'.bold} debugger.`,
        requiresArg: true,
        type: 'number'
      }
    }, _.pick(OPTIONS, ['projectId', 'region', 'host', 'restPort'])))
...
exports.handler = (opts) => {
  ...
  return controller.doIfRunning()
    .then(() => controller.debug('debug', opts.functionName, opts))
